...
function simulateTOY_ncellsScenarioNMOD(ncells::Int64,ncellsI::Int64,ncellsC::Int64,Iapp_cell,Istep_cell::Vector{Float64},gCAMPA::Matrix{Float64},  w_init::Matrix{Float64}, idxPrePost::Vector{CartesianIndex{2}})
  ...
  for z = 1:Tdt
          ...
          for k=1:1:nPre
            ...
            if(state[z]<2) #testing phase > aucun changement de w
              ...
              #  BURST
                  ...
                  if(ca_var[k,jj]>= theta_p)
                    ww_var[k,jj] +=(dt)*(1/tauw_p)*(Omega_p_burst[k,jj] - ww_var[k,jj])
                    w_dot[k,jj]=(1/tauw_p)*(Omega_p_burst[k,jj] - ww_var[k,jj])
                  elseif(ca_var[k,jj]>=theta_d && ca_var[k,jj]<theta_p)
                    ww_var[k,jj] +=(dt)*(1/tauw_d)*(Omega_d - ww_var[k,jj])
                    w_dot[k,jj] =(1/tauw_d)*(Omega_d - ww_var[k,jj])
                  else
            ...
            if state[z] <2
              if state[z] == 1 #|| state[z]==-1
                #update de g en fontion de w_dot
                gCAMPA_var[k,jj] +=(dt)*(-w_dot[k,jj])*1/tauL 


                if(gCAMPA_var[k,jj]>=2) 
                    gCAMPA_var[k,jj]=2
                end
    ...
    if(mod(z,1/dt)==0) #chaque fois que z est multiple de 1/dt
      ...
      ca_var_plot[:,idx_w]  = copy(reshape(ca_var, length(ca_var), 1))
      #cpre_var_plot[:,idx_w]  = copy(reshape(cpre_var, length(cpre_var), 1))
      #cpost_var_plot[:,idx_w] = copy(reshape(cpost_var, length(cpost_var), 1))

      w_dot_plot[:,idx_w]    = copy(reshape(w_dot, length(w_dot), 1))
