...
directory_name = "/Users/kathleen/Documents/PhD/2023-Project/Fig3_Model"
...
include("model_Graupner2012.jl")
include("PARAMS_cycle.jl")burst
const new_network=0

const bound_type = "SB" 
const region = "Cortex" # or HPC
const wMAX = 1
const wMIN = 0
...
if(new_network==1)
    ...
    neurons_freq[:,2:end] = round.(rand(Uniform(0,70),N_samples*N_cycles, ncells-1))
...
if(bound_type=="SB")
    ...
    const Omega_burst = Omega_p #0.75
    const Omega_d = 0.
    const Omega_0 = 0.
    ...
    const mu=0
    const Omega_p = gamma_p*0.5-gamma_d*0.5
    const Omega_burst = -gamma_d*0.5
    const Omega_d = 0.
    const Omega_0 = 0.
...
@time (Vspk, Vspk_bin,) = simulateTOY_ncellsScenarioNMOD(
    ncells,
    ncellsI,
    ncellsC,
    Iapp_cell,
...
#=
writedlm(@sprintf("%s/data/%s/%s/%s/pot_tonic.dat",directory_name, experiment_name, region, bound_type), pot_tonic, header=false)
writedlm(@sprintf("%s/data/%s/%s/%s/dep_tonic.dat",directory_name, experiment_name, region, bound_type), dep_tonic, header=false)
writedlm(@sprintf("%s/data/%s/%s/%s/null_tonic.dat",directory_name, experiment_name, region, bound_type), null_tonic, header=false)

writedlm(@sprintf("%s/data/%s/%s/%s/pot_burst.dat",directory_name, experiment_name, region, bound_type), pot_burst, header=false)
writedlm(@sprintf("%s/data/%s/%s/%s/dep_burst.dat",directory_name, experiment_name, region, bound_type), dep_burst, header=false)
writedlm(@sprintf("%s/data/%s/%s/%s/null_burst.dat",directory_name, experiment_name, region, bound_type),null_burst, header=false)
=#
