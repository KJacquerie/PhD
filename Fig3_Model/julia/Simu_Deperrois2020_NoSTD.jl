...
using CSV
using LinearAlgebra
using Distributionsburst
using DataStructures
using DSP

## Include model

include("model_Deperrois2020_NoSTD.jl")
...
const SD = "no"
const experiment_name = "Deperrois2020_NoSTD"
...
if(new_network==1)
    ...
    neurons_freq[:,2:end] = round.(rand(Uniform(0.1,70),N_samples*N_cycles, ncells-1))
...
const expm = "Control"
if(region =="Cortex")
    const tau_Ca = 32.1900754   # 22.27212 #[ms]
    const C_Pre  = 1.60681037   # 0.84410
    const C_Post = 1.1243642    # 1.62138
    const D_pre  = 5.75272377   # 9.53709 #[ms]
    
    const tau_w    = 79.9756573e3 # 520.76129e3 #[s>ms]
    const gamma_p  = 161.987985 # 597.08922
    const gamma_d  = 31.9759883 # 137.7586
    const n_lin = 1.
