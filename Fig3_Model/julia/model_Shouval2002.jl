...
function Sigma_Shouval(Ca::Float64) 
  if(mu==1)
    res = a0 - a0.* exp(b1.*(Ca-a1))./(1+exp(b1*(Ca-a1))) + m2*exp(b2.*(Ca-a2))./(1+exp(b2.*(Ca-a2)))
  else #mu=0
    res = m2*exp(b2*(Ca-a2))/(1+exp(b2*(Ca-a2)))  - m1*exp(b1*(Ca-a1))/(1+exp(b1*(Ca-a1)))
  end
  return res
end

tau_Shouval(Ca::Float64) = P4+ P1./(P2+(Ca).^P3)
    ...
    for j=ncellsI+1:ncellsC+ncellsI
        ...
        # Postsynaptic Excitatory cells / Second layer
          ...
          for k=1:1:nPre
            ...
            ww_var[k,jj] += (dt)*(1/tau_Shouval(ca_var[k,jj]))*(Sigma_Shouval(ca_var[k,jj])-mu*ww_var[k,jj])
