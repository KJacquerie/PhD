...
include("model_MNIST_GB2012_learn.jl")
const experiment_name = "Graupner2012_learn"
#model = "NEW" #or OLD
new_circuit=0
...
for idx_state= 1:1:N_states*N_samples-1
    # ajouter un nouveau chiffre à apprendre pendant les états pairs
        if(mod(idx_state,2)==0)
            idx_presented[idx_state] = convert(Int64,idx_presented[idx_state]+19)
...
neurons_freq[:,1].=1  
neurons_freq_learn = readdlm(@sprintf("%s/data/neurons_freq_learn.dat",directory_name))
#writedlm(@sprintf("%s/data/neurons_freq_learn.dat",directory_name), neurons_freq, header=false)
