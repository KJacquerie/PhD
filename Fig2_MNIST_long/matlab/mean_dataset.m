...
for idx=1:1:NB_dgt
    mean_data(:,idx) = mean(dataset_MNIST(:,(idx-1)*N+1:1:N*idx-30),2); 
    mean_data_learn(:,idx) = mean(dataset_MNIST(:,(idx-1)*N+1:1:N*idx-11),2); 
    mean_data_full(:,idx) = mean(dataset_MNIST(:,(idx-1)*N+1:N*idx),2); 
    X = reshape(mean_data(:,idx), [NB_neurons,NB_neurons]);
    X_full = reshape(mean_data(:,idx), [NB_neurons,NB_neurons]);
    X_learn = reshape(mean_data_learn(:,idx), [NB_neurons,NB_neurons]);
    Y = conv2(X,F);
    figure(idx)
    subplot(1,2,1)
    imagesc(X')
...
dlmwrite('mean_MNIST_full.dat', mean_data_full,'-append','delimiter',' ','roffset',1)
dlmwrite('mean_MNIST_20.dat', mean_data,'-append','delimiter',' ','roffset',1)
dlmwrite('mean_MNIST_learn.dat', mean_data_learn,'-append','delimiter',' ','roffset',1)
