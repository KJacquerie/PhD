...
experiment_name = "Graupner2012_silent"; 
bound_type = "SB";
fMax=50; 
new_curves=1; 
Fig_num = "Fig2_SI"; 
...
print(sprintf('%s/%s/fig/%s_w',directory_name, Fig_num, experiment_name), '-dsvg', '-painters')


mm_MM = [mm MM]; 
csvwrite(sprintf('%s/%s/fig/%s_mm_MM',directory_name, Fig_num, experiment_name), mm_MM)
...
MM = max(max(l_rand)); 
if(mm~=MM)
    if(strcmp(experiment_name, 'Graupner2012_silent'))
        ylim([0.0009 MM]); 
    else
        ylim([mm MM]); 
    end
...
set(gcf, 'PaperUnits', 'centimeters');
set(gcf, 'PaperPosition', [0 0 ptx_long pty]);
print(sprintf('%s/%s/fig/%s_l',directory_name, Fig_num,experiment_name), '-dsvg', '-painters')

if(strcmp(experiment_name, 'Graupner2012_silent'))
    mm_MM = [0.0009 MM]; %for silent
else
    mm_MM = [mm MM]; %for other models
end


csvwrite(sprintf('%s/%s/fig/%s_l_mm_MM',directory_name, Fig_num, experiment_name), mm_MM)
...
set(gcf, 'PaperUnits', 'centimeters');
set(gcf, 'PaperPosition', [0 0 ptx_long pty]);
print(sprintf('%s/%s/fig/%s_wl',directory_name, Fig_num,experiment_name), '-dsvg', '-painters')


mm_MM = [mm MM]; 
csvwrite(sprintf('%s/%s/fig/%s_wl_mm_MM',directory_name, Fig_num, experiment_name), mm_MM)
...
print(sprintf('%s/%s/fig/Raster_%s', directory_name, Fig_num, experiment_name), '-dsvg', '-painters')
...
print(sprintf('%s/%s/fig/LFP_%s', directory_name, Fig_num, experiment_name), '-dsvg')
