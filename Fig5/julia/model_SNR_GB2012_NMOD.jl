...
function simulateTOY_ncellsScenarioNMOD(ncells::Int64,ncellsI::Int64,ncellsC::Int64,Iapp_cell,Istep_cell::Vector{Float64},gCAMPA::Matrix{Float64})
  ...
  for z = 1:Tdt
        ...
        # Postsynaptic Excitatory cells / Second layer
          ...
          for k=1:1:nPre
            ...
            # BURST
              ...
              # STRUCUTRAL PLASTICITY -- NMOD
              if(k<6)
                gCAMPA_var[k,jj] +=(dt)*(-w_dot[k,jj])*1/tauG_corr
              else
                gCAMPA_var[k,jj] +=(dt)*(-w_dot[k,jj])*1/tauG_uncorr
              end


                if(gCAMPA_var[k,jj]>=(0.2)) 
                    gCAMPA_var[k,jj]=(0.2)
                end
    ...
    if(mod(z,1/dt)==0)
      #VV[:,idx_w] = copy(V)
      ww[:,idx_w] = copy(reshape(ww_var[1:10], 10, 1))
      #ww[:,idx_w] = copy(temp[idx_wl])
      
      gCAMPA_plot[:,idx_w] = copy(reshape(gCAMPA_var[1:10], 10, 1))
    ...
    if(z==Tdt_StateTime[idx_state])
      ...
      writedlm(@sprintf("%s/data/%s/w_BACK.dat",directory_name, experiment_name), w_BACK, header=false)
      writedlm(@sprintf("%s/data/%s/g_BACK.dat",directory_name, experiment_name), g_BACK, header=false)
