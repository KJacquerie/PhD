...
function simulateTOY_ncellsScenarioNMOD(ncells::Int64,ncellsI::Int64,ncellsC::Int64,Iapp_cell,Istep_cell::Vector{Float64},gCAMPA::Matrix{Float64})
  ...
  for z = 1:Tdt
    ...
    if(z==Tdt_StateTime[idx_state])
      ...
      writedlm(@sprintf("%s/data/%s/tau=%d/w_BACK.dat",directory_name, experiment_name, tauG), w_BACK, header=false)
      writedlm(@sprintf("%s/data/%s/tau=%d/g_BACK.dat",directory_name, experiment_name, tauG), g_BACK, header=false)
