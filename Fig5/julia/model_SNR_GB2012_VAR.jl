...
###
function simulateTOY_ncellsScenarioNMOD(ncells::Int64,ncellsI::Int64,ncellsC::Int64,Iapp_cell,Istep_cell::Vector{Float64},gCAMPA_init::Float64, tauG, gCAMPA_init_idx)
  # Initial conditions
  V=-60*ones(ncells)
  ...
  gCAMPA = gCAMPA_init*ones(nPre, nPost)
  gCAMPA_var = copy(gCAMPA)
  gCAMPA_plot = zeros(length(ww_var),convert(Int64,T))
  g_BACK = zeros(length(ww_var), N_cycles*2)
  ...
  for z = 1:Tdt
    ...
    if(mod(z,1/dt)==0)
      ...
      #ww[:,idx_w] = copy(temp[idx_wl])
      
      #gCAMPA_plot[:,idx_w] = copy(reshape(gCAMPA_var, length(ww_var), 1))
      #gCAMPA_plot[:,idx_w] = copy(temp[idx_wl])
    ...
    if(z==Tdt_StateTime[idx_state])
      ...
      writedlm(@sprintf("%s/data/%s/%s/tau=%d/%d/w_BACK.dat",directory_name, experiment_name, experiment_type, tauG, gCAMPA_init_idx), w_BACK, header=false)
      writedlm(@sprintf("%s/data/%s/%s/tau=%d/%d/g_BACK.dat",directory_name, experiment_name, experiment_type, tauG, gCAMPA_init_idx), g_BACK, header=false)
  ...
  #return  ww,  gCAMPA_plot, Spkt, Spkt_binary, LFP_C
end
