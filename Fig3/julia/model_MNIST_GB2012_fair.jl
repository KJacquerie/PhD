...
###
#function simulateTOY_ncellsScenarioNMOD(ncells::Int64,ncellsI::Int64,ncellsC::Int64,Iapp_cell,Istep_cell::Vector{Float64},gCAMPA::Matrix{Float64},  w_init::Matrix{Float64}, pixel_ON)
function simulateTOY_ncellsScenarioNMOD(ncells::Int64,ncellsI::Int64,ncellsC::Int64,Istep_cell::Vector{Float64},gCAMPA::Matrix{Float64},  w_init::Matrix{Float64}, neurons_freq_DGT, wg_min, wg_max, wg_scale)
...
# Initial conditions
  ...
  wg_var = ww_var.*gCAMPA_var
  wg_norm = wg_scale.*(wg_var.-wg_min)./(wg_max-wg_min)

  #ImageView.imshow(reshape(ww_var[:,1], 22, 22).*reshape(gCAMPA_var[:,1], 22, 22))
  ...
  Iapp_cell[:,1:Tdt_state] = get_Iapp(Duration_sample, dt, neurons_freq_DGT, spike_duration)
  z_Iapp=1
  Iappstep = 0.
    ...
    for j=ncellsI+1:ncellsC+ncellsI
        ...
        # Postsynaptic excitatory cells | Layer 2
          ...
          for k=1:1:nPre
            ...
            V[j] += (dt)*(1/C)*(-wg_norm[k,jj]*AMPA[k]*(Vprev[j]-0))


            
          end
          if(V[j]>=0. && Vprev[j]<0. && z>= Tdt_transient)
            Spkt[jj] = Spkt[jj]+1
          end
        end
