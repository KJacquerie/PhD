...
function simulateTOY_ncellsScenarioNMOD(ncells::Int64,ncellsI::Int64,ncellsC::Int64,Iapp_cell,Istep_cell::Vector{Float64},gCAMPA::Matrix{Float64})
  ...
  # Omega_burst*ones(nPre, nPost)
  ww_var = ones(nPre, nPost)*0.5
  ww=zeros(length(idx_wl),convert(Int64,T))
  w_BACK = zeros(length(ww_var), N_cycles*2)

  w_dot = zeros(nPre,nPost)

  gCAMPA_var = copy(gCAMPA)
  gCAMPA_plot = zeros(length(idx_wl),convert(Int64,T))
  ...
  for z = 1:Tdt
        ...
        # Postsynaptic Excitatory cells / Second layer
          ...
          for k=1:1:nPre
            ...
            # BURST
              ...
              # |  
              # |     STRUCTURAL PLASTICITY IS BLOCKED
              # |
    ...
    if(mod(z,1/dt)==0)
      VV[:,idx_w] = copy(V)
      temp = copy(reshape(ww_var, length(ww_var), 1))
      ww[:,idx_w] = copy(temp[idx_wl])
      
      temp = copy(reshape(gCAMPA_var, length(gCAMPA_var), 1))
  ...
  return  ww, gCAMPA_plot, VV, Spkt, Spkt_binary, LFP_C
end
