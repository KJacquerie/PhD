    ...
    for j=ncellsI+1:ncellsC+ncellsI
        ...
        if(j>ncellsI+nPre)
          ...
          for k=1:1:nPre      # (k)  PRE
            if pre_spikes[k] ==1
              ...
              if SB == 0 
                new = -y_prev[ll]*Am  + ww_var[k,ll]
              else
                new = -y_prev[ll]*Am* ww_var[k,ll] + ww_var[k,ll]
              end
              ww_var[k,ll] = new
              if(ww_var[k,ll]> wMax)
                ww_var[k,ll] = wMax
        ...
        if(j>ncellsI && j<=ncellsI+nPre)
          ...
          for k=1:1:nPost # post
            ...
            if post_spikes[k] ==1
              if SB==0
                new = x_prev[ll]*Ap +ww_var[ll,k] 
              else
                new = x_prev[ll]*Ap*(1-ww_var[ll,k]) + ww_var[ll,k]
              end
              ww_var[ll,k] = new
              if ww_var[ll,k] > wMax
                ww_var[ll,k] = wMax
