...
function simulateTOY_ncellsScenarioNMOD(ncells::Int64,ncellsI::Int64,ncellsC::Int64,Iapp_cell,Istep_cell::Vector{Float64},gCAMPA::Matrix{Float64})
  ...
  pot_mat = zeros(nPre, nPost)
  dep_mat = zeros(nPre, nPost)
  nul_mat = zeros(nPre, nPost)


  demo_pot = zeros(length(ww_var),N_cycles*2)
  demo_dep = zeros(length(ww_var),N_cycles*2)
  demo_nul = zeros(length(ww_var),N_cycles*2)
  ...
  for z = 1:Tdt
        ...
        # Postsynaptic Excitatory cells / Second layer
          ...
          for k=1:1:nPre
            ...
            # SLEEP
              ...
              if(ca_var[k,jj]>= theta_p)
                ww_var[k,jj] +=(dt)*(1/tauw_p)*(Omega_p_sleep[k,jj] - mu*ww_var[k,jj])
                pot_mat[k,jj] +=1
              elseif(ca_var[k,jj]>=theta_d && ca_var[k,jj]<theta_p)
                ww_var[k,jj] +=(dt)*(1/tauw_d)*(Omega_d - mu*ww_var[k,jj])
                dep_mat[k,jj] +=1
    ...
    if(mod(z,1/dt)==0)
      ...
      Ca_plot[:,idx_w] = copy(reshape(ca_var[1:10,1:10], 100, 1))
    ...
    if(z==Tdt_StateTime[idx_state])
      ...
      demo_pot[:,idx_state] = copy(reshape(pot_mat, length(ww_var), 1))
      demo_dep[:,idx_state] = copy(reshape(dep_mat, length(ww_var), 1))
      demo_nul[:,idx_state] = copy(reshape(nul_mat, length(ww_var), 1))

      writedlm(@sprintf("%s/data/%s/%s/%s/demo_pot.dat",directory_name, experiment_name, region, bound_type), demo_pot, header=false)
      writedlm(@sprintf("%s/data/%s/%s/%s/demo_dep.dat",directory_name, experiment_name, region, bound_type), demo_dep, header=false)
      writedlm(@sprintf("%s/data/%s/%s/%s/demo_nul.dat",directory_name, experiment_name, region, bound_type), demo_nul, header=false)
  ...
  return  ww, Spkt, Spkt_binary,Ca_plot
end
