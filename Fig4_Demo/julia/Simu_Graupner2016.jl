...
directory_name = "/Users/kathleen/Documents/PhD/2023-reset/Fig4_Demo"
...
const SD = "no"
if(SD=="yes")
    experiment_name = "Graupner2016_SD"
else
...
const w0rand = rand(nPre,nPost)
writedlm(@sprintf("%s/data/w0rand.dat",directory_name), w0rand, header=false)
...
const expm = "Control"
if(region =="Cortex")
    #_____________________________  Cortex
    ...
    #_____________________________  HPC
	const tau_Ca = 22.27212 #[ms]
	const C_Pre  = 0.84410
	const C_Post = 1.62138
	const D_pre  = 9.53709 #[ms]
...
if(bound_type=="SB")
    ...
    const Omega_sleep = Omega_p #0.75
    const Omega_d = 0.
    const Omega_0 = 0.
...
@time (w, Vspk, Vspk_bin, Ca) = simulateTOY_ncellsScenarioNMOD(
    ncells,
    ncellsI,
    ncellsC,
    Iapp_cell,
...
#writedlm(@sprintf("%s/data/%s/%s/V.dat",directory_name, experiment_name, bound_type), VV, header=false)
#writedlm(@sprintf("%s/data/%s/%s/V1.dat",directory_name, experiment_name, bound_type), V1, header=false)
#writedlm(@sprintf("%s/data/%s/%s/V2.dat",directory_name, experiment_name, bound_type), V2, header=false)
...
writedlm(@sprintf("%s/data/%s/%s/%s/Ca.dat",directory_name, experiment_name, region, bound_type), Ca, header=false)
#writedlm(@sprintf("%s/data/%s/%s/LFP.dat",directory_name, experiment_name, bound_type), LFP_C_filt, header=false)
