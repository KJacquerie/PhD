...
experiment_mat = ["Graupner2012", "Graupner2012_RESET_strong", "Graupner2012_noBURST_strong"]; 
...
TYPE_NORM = 'ABS'; % 'ABS' / 'REL';

chosen_STATE=[8,9, NB_states-3, NB_states-2, NB_states-1, NB_states]; 
for idx_expm = 1:1:length(experiment_mat)
    ...
    for idx_chosen = 1:1:length(chosen_STATE)
        ...
        for idx_dgt = 1:1:NB_dgt
            ...
            subplot(length(experiment_mat), NB_dgt, count+(idx_expm-1)*10)
...
for idx_expm=1:1:length(experiment_mat)
    ...
    for idx = 1:1:NB_states
        for idx_dgt = 1:1:NB_dgt
            ...
            g_shaped = reshape(g((idx_dgt-1)*nPre+1:idx_dgt*nPre,idx ),NB_grid,NB_grid); 
            wg_shaped = (w_shaped.*g_shaped - wgMIN)/(wgMAXX - wgMIN);
            Temp = corr2(mean_cell{idx_dgt}, w_shaped.*g_shaped); 
...
for idx_dgt=1:1:NB_dgt
    ...
    if(isnan(x)==0)
        for idx_x=1:1:length(x)
            if(mod(idx_x,2)==1)
                plot([x(idx_x) x(idx_x)], [-1 1], '-', 'color', [0.5 0.5 0.5])
            end
        end
    end
    ...
    plot([1:1:NB_states], Err_Model2(:,idx_dgt), '-o', 'color', [1 0.5 0])
    plot([1:1:NB_states], Err_Model3(:,idx_dgt), '-o', 'color', [0.7 0 1])
    ylim([-1 1])
    xlim([0 32])
    set(figure(idx_dgt),'PaperPositionMode','auto');
    set(figure(idx_dgt), 'PaperUnits', 'centimeters');
    set(figure(idx_dgt), 'PaperPosition', [0 0 40 5]);
    print(sprintf('%s/fig/ERR_%d',directory_name, idx_dgt),'-depsc', '-painters')
end
